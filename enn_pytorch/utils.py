...
def epistemic_network_from_module(
    ...
    def apply_fn(model: nn.Module, inputs: torch.Tensor, index: base.Index) -> base.Output:
...
def make_batch_iterator(
    data: base.Batch, batch_size: Optional[int] = None, seed: int = 0, num_workers: int = 0, device: str = 'cuda:0'
) -> base.BatchIterator:
    ...
    x = torch.tensor(data.x, device=device) if isinstance(data.x, np.ndarray) else data.x.to(device)
    y = torch.tensor(data.y, device=device) if isinstance(data.y, np.ndarray) else data.y.to(device)
    data_index = torch.tensor(data.data_index, device=device) if isinstance(data.data_index, np.ndarray) else data.data_index.to(device)
    weights = torch.tensor(data.weights, device=device) if isinstance(data.weights, np.ndarray) else data.weights.to(device)
