...
from enn_pytorch import data_noise
...
def average_single_index_loss(
    ...
    def loss_fn(
        ...
        # Average losses
        mean_loss = torch.stack(losses).mean() if isinstance(losses[0], torch.Tensor) else sum(losses) / len(losses)
...
class BatchedL2Loss(SingleIndexLossFn):
    ...
    def __call__(
        ...
        if batch.weights is None:
            ...
            batch_weights = batch.weights if isinstance(batch.weights, torch.Tensor) else torch.tensor(batch.weights, dtype=torch.float32)
