...
from enn_pytorch.experiments.neurips_2021 import testbed
from enn_pytorch.ntk import Dense, Relu, Serial
from enn.experiments.neurips_2021.load import make_benchmark_kernel as jax_make_benchmark_kernel
...
class RegressionTestbedConfig:
    ...
    kernel_ctor: Callable[[int], object] = lambda input_dim: make_benchmark_kernel(input_dim)
...
def regression_load(
    input_dim: int, data_ratio: float, seed: int, noise_std: float, dataset_folder: str = "datasets"
) -> testbed_base.TestbedProblem:
    ...
    filepath = os.path.join(
        dataset_folder, f"regression_id{input_dim}dr{data_ratio}ns{noise_std}seed{seed}.pt"
    )
