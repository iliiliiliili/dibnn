...
from enn_pytorch.experiments.neurips_2021.base import Data, ENNQuality, EpistemicSampler, PriorKnowledge, TestbedProblem
from enn_pytorch.ntk import gradient_descent_mse_ensemble
...
class GPRegression:
    ...
    @staticmethod
    def restore(tau, input_dim, x_train, x_test, x_val, train_y, test_mean, test_cov, val_mean, val_cov, noise_std, kernel_ridge):
        result = GPRegression(
...
class TestbedGPRegression(TestbedProblem):
    ...
    def evaluate_quality(
        ...
        kl_estimates = torch.stack(
            [_kl_gaussian(posterior_mean[i], posterior_std[i], enn_mean[i], enn_std[i])
        ...
        result = ENNQuality(
            kl_estimate.item(), {"mean_error": error_mean.item(), "std_error": error_std.item()}
        )

        return result

    def evaluate_quality_val(
        self, enn_sampler: EpistemicSampler
    ) -> ENNQuality:
        """Computes KL estimate on mean functions for tau=1 only."""
...
def _kl_gaussian(mean_1: float, std_1: float, mean_2: float, std_2: float) -> torch.Tensor:
