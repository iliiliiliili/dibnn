...
from enn_pytorch.ntk.core import add_diagonal_regularizer, canonicalize_axis, canonicalize_get, get_attr, get_axes, get_cho_solve, get_dependency, get_first, make_2d, make_expm1_fn, make_inv_expm1_fn, zip_axes
from enn_pytorch.ntk.kernel import KernelLike
...
def gradient_descent_mse_ensemble(
    ...
    def predict_fn(
        ...
        k_dd, k_td, nngp_tt = get_kernels(
            get, x_test, compute_cov
        )
                    ...
                    if g == "nngp":
                        cov = torch.einsum(
                            ...
                            (
                                ...
                                * torch.exp(-2 * torch.clamp(evals, min=0.0) * t / y_train.numel())
                            ),
                            evecs,
                        )
                        ...
                        exp = torch.einsum(
                            "mi,ti,ki->tmk",
                            evecs,
                            torch.exp(-torch.clamp(evals, min=0.0) * t / y_train.numel()),
                            evecs,
                        )
                        cov = torch.einsum(
                            "tmk,kl,tnl->tmn", exp, nngp_dd, exp
                        )
