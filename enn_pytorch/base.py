...
class OutputWithPrior(NamedTuple):
    ...
    def preds(self) -> torch.Tensor:
        if isinstance(self.train, torch.Tensor) and isinstance(self.prior, torch.Tensor):
            return self.train + self.prior.detach()
        elif isinstance(self.train, torch.Tensor):
            return self.train + torch.tensor(self.prior, device=self.train.device, dtype=self.train.dtype).detach()
        else:
...
class Batch(NamedTuple):
    ...
    weights: Optional[torch.Tensor] = None  # None should default to weights = torch.ones
