...
def make_dropout_ctor(
    ...
    def enn_ctor(
        prior: testbed_base.PriorKnowledge, use_double_precision: bool = False
    ):
        output_sizes = (
...
def make_ensemble_ctor(
    ...
    def make_agent_config() -> agents.VanillaEnnConfig:
        config = agents.VanillaEnnConfig(
            ...
            early_stopping_min_delta=early_stopping_min_delta,
            early_stopping_eval_freq=early_stopping_eval_freq,
            fixed_sampler_for_training=True,
        )
...
def make_dropout_best_sweep() -> List[AgentCtorConfig]:
    ...
    for dropout_rate in [0.2]:
        for regularization_scale in [1e-4]:
            for num_layers in [2]:
...
def make_agent_sweep(
    agent: str = "all", reduce_batch=False
) -> Sequence[AgentCtorConfig]:
    ...
    if agent == "all":
        agent_sweep = (
            ...
            + make_bbb_sweep()
            + make_ensemble_sweep()
            + make_hypermodel_sweep(reduce_batch=reduce_batch)
            + make_vnn_sweep(reduce_batch=reduce_batch)
            + make_layer_ensembles_sweep(reduce_batch=reduce_batch)
        )
    if agent == "all_best":
        agent_sweep = (
            make_dropout_best_sweep()
            + make_bbb_best_sweep(reduce_batch=reduce_batch)
            + make_ensemble_best_sweep()
            + make_hypermodel_best_sweep(reduce_batch=reduce_batch)
        )
    elif agent == "dropout":
        ...
        agent_sweep = make_bbb_sweep(reduce_batch=reduce_batch)
    elif agent == "bbb_best":
...
def load_agent_config(
    agent_id: int, agent: str = "all", reduce_batch=False
) -> agents.VanillaEnnConfig:
