...
def convert_enn_to_prior_fn(enn: base.EpistemicNetwork, key: base.RngKey) -> PriorFn:
...
def make_null_prior(output_dim: int) -> Callable[[torch.Tensor], torch.Tensor]:
    ...
    def null_prior(inputs: torch.Tensor) -> torch.Tensor:
        if isinstance(inputs, torch.Tensor):
            return torch.zeros(
                [inputs.shape[0], output_dim], dtype=torch.float32, device=inputs.device
            )
        else:
...
def _parse_gamma(
    ...
    if isinstance(gamma, float) or isinstance(gamma, int):
        ...
        if isinstance(key, int):
            ...
            return gamma_min + (gamma_max - gamma_min) * torch.rand(
                1, num_feat, 1, generator=key
            )
