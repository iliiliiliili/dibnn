...
from src.ntk.core import zip_axes
...
class Kernel:
    ...
    def __init__(
        ...
        self.nngp = self.__compute_covariance(
            x1, x1 if x2 is None else x2, full_batch=True
        )
        self.ntk = None
    ...
    def __compute_covariance_diagonal_batch_diagonal_spatial(self, x):
        ...
        new_batch_axis = self.batch_axis - (
            1 if self.batch_axis > self.channel_axis else 0
        )
    ...
    def __compute_covariance_diagonal_batch_full_spatial(self, x):
        ...
        spatial_shape = [
            x_reordered.shape[i] for i in range(1, len(x_reordered.shape) - 1)
        ]
    ...
    def __compute_covariance_full_batch_full_spatial(
        ...
        new_batch_axis = self.batch_axis - (
            1 if self.batch_axis > self.channel_axis and self.channel_axis >= 0 else 0
        )
        ret = torch.moveaxis(
            ret, (new_batch_axis, x1.ndim - 1 + new_batch_axis), (0, 1)
        )
        ret = zip_axes(ret, 2)
    ...
    def __compute_covariance_full_batch_diagonal_spatial(
        ...
        raise NotImplementedError(
            "Full batch, diagonal spatial covariance not implemented yet."
        )
