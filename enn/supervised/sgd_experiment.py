...
class MultiIndexerExperiment(supervised_base.BaseExperiment):
    """Class to handle supervised training.
    ...
    def __init__(
        ...
        def create_forward(indexer_id: str):
        ...
        for id in indexers.keys():
            self.forwards[id] = create_forward(id)
    ...
    def predict(self, inputs: base.Array, seed: int, indexer_id: str) -> base.Array:
        """Evaluate the trained model at given inputs."""
        return self.forwards[indexer_id](
            self.state.params, inputs, jax.random.PRNGKey(seed)
        )
...
class BatchedExperiment(supervised_base.BaseExperiment):
    """Class to handle supervised training.
    ...
    def __init__(
        ...
        def forward(
            ...
            batched_apply = jax.vmap(self.enn.apply, in_axes=[None, None, 0])
        ...
        self._forward = jax.jit(
            batched_forward if self.batched_inference else forward, static_argnums=(3,)
        )
    ...
    def predict(self, inputs: base.Array, seed: int, num_samples: int) -> base.Array:
        """Evaluate the trained model at given inputs."""
        return self._forward(
            self.state.params, inputs, jax.random.PRNGKey(seed), num_samples
        )
...
class BatchedRankedExperiment(supervised_base.BaseExperiment):
    """Class to handle supervised training.
    ...
    def __init__(
        ...
        def forward(params: hk.Params, inputs: base.Array, samples) -> base.Array:
            batched_apply = jax.vmap(self.enn.apply, in_axes=[None, None, 0])
            result = batched_apply(params, inputs, samples)
            return result

        def batched_forward(
            params: hk.Params, inputs: base.Array, samples
        ) -> base.Array:
            result = self.enn.apply(params, inputs, samples)
            return result

        self._forward = jax.jit(batched_forward if self.batched_inference else forward)
    ...
    def predict(self, inputs: base.Array, samples) -> base.Array:
        """Evaluate the trained model at given inputs."""
        return self._forward(self.state.params, inputs, samples)
