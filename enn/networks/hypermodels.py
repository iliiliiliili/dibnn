...
def hypermodel_module(
    ...
    def hyper_fn(inputs: base.Array, index: base.Index) -> base.Array:
        ...
        if diagonal_linear_hyper:
            ...
            flat_output = jax.tree_unflatten(
                jax.tree_structure(base_shapes), flat_output
            )
        else:
        ...
        if scale:
            ...
            generated_params_scaled = hk.data_structures.map(scale_fn, generated_params)
        else:
...
class MLPHypermodelWithHypermodelPrior(base.EpistemicNetwork):
    ...
    def __init__(
        ...
        def prior_net(x):
            return hk.nets.MLP(prior_base_output_sizes, w_init=w_init, b_init=b_init)(x)
...
class HyperLinear(hk.Module):
    ...
    def __call__(self, x: base.Array, z: base.Index) -> base.Array:
        ...
        w = hk.get_parameter(
            "w", [self._output_size, hidden_size, self._index_dim_per_layer], init=init
        )
        b = hk.get_parameter(
...
class PriorMLPIndependentLayers(hk.Module):
    ...
    def __init__(
        ...
        # indices
        self._layers = []
        for layer_indices, output_size in zip(self._layers_indices, self._output_sizes):
            index_dim_per_layer = len(layer_indices)
            layer = HyperLinear(
                output_size,
                index_dim_per_layer,
                self._weight_scaling,
                self._bias_scaling,
