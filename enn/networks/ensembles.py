    ...
    def __init__(
        ...
        def flat_mlp():
            return hk.Sequential(
                [hk.Flatten(), hk.nets.MLP(output_sizes, w_init=w_init, b_init=b_init)]
            )
...
def make_mlp_ensemble_prior_fns(
    ...
    def net_fn(x):
        layers = [hk.Flatten(), hk.nets.MLP(output_sizes, w_init=w_init, b_init=b_init)]
    ...
    for _ in range(num_ensemble):
        prior_fns.append(
            priors.make_random_feat_gp(
                input_dim, output_dim, num_feat, next(rng), gamma, scale=1.0
            )
        )
    return prior_fns
